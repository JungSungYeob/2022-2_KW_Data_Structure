...
bool BFS(Graph *graph, int vertex) {
    if (vertex >= graph->getSize())//if vertex>=graph size
        return false;
    map<int, int> *tmp;
...
bool DFS_R(Graph *graph, vector<bool> &visited, int vertex) {
    if (vertex >= graph->getSize())
        return false;//if vertex is out of range
    map<int, int> *tmp;
    tmp = new map<int, int>[graph->getSize()];//generate new map
    visited[vertex] = true;//visited true of first vertex
    ofstream flog;
    flog.open("log.txt", ios::app);
    flog << "==========DFS_R==========" << endl;
    flog << "startvertex: " << vertex << endl;
    int cur = vertex;//cur is first vertex
    flog << cur;
    flog.close();//file close to write txt
    graph->getAdjacentEdges(cur, tmp);//get cur vertex adjacent edges
    for (auto iter = tmp[cur].begin(); iter != tmp[cur].end(); iter++) {

        int next = iter->first;
        if (!visited[next]) {
            recursion(graph, visited, next);//recursion for DFS_R
        }
    }
    flog.open("log.txt", ios::app);
    flog << "\n=======================" << endl;
...
bool recursion(Graph *graph, vector<bool> &visited, int vertex) {
    ...
    int cur = vertex;
    flog << " -> " << vertex;
    flog.close();//file close to write
    graph->getAdjacentEdges(cur, tmp);//get cur vertex adjacent edges
    for (auto iter = tmp[cur].begin(); iter != tmp[cur].end(); iter++) {
...
bool Kruskal(Graph *graph) {
    ...
    quicksort(&sort_tmp, 0, sort_tmp.size() - 1);//sort by quicksort or insertion sort
...
bool Dijkstra(Graph *graph, int vertex) {
    ...
    for (int i = 0; i < graph->getSize(); i++) {
        graph->getDirectedEdges(i, tmp);//get all directed edges
        distance[i] = INF;//set all INFINITY
        prev[i] = INF;//set all INFINITY
    }
...
bool Bellmanford(Graph *graph, int s_vertex, int e_vertex) {
    ...
    for (int i = 0; i < graph->getSize(); i++) {
        graph->getDirectedEdges(i, tmp);//get all directed edges
        distance[i] = INF;//reset all distance and prev infinity
        prev[i] = INF;
    }
    ...
    for (int i = 2; i < graph->getSize(); i++) {//k=2~k=graph size
        for (int j = 0; j < graph->getSize(); j++) {
            for (auto it_ = tmp[j].begin(); it_ != tmp[j].end(); it_++) {
                ...
                if (distance[j] == INF)//if not connect
                    continue;
...
bool FLOYD(Graph *graph) {
    ...
    for (int i = 0; i < graph->getSize(); i++) {
        graph->getDirectedEdges(i, tmp);
    }
    for (int i = 0; i < graph->getSize(); i++) {
        tmpmat[i] = new int[graph->getSize()];//generate pointer array
        for (int j = 0; j < graph->getSize(); j++) {
            tmpmat[i][j] = INF;//set all infinity
            if (i == j)
    ...
    for (int i = 0; i < graph->getSize(); i++) {
        for (int j = 0; j < graph->getSize(); j++) {
            for (int k = 0; k < graph->getSize(); k++) {
                if(tmpmat[i][k]==INF||tmpmat[j][i]==INF)
                    continue;
                int temp = tmpmat[j][i] + tmpmat[i][k];
                if (temp < tmpmat[j][k])//if tmpmat[j][i]+tmpmat[i][k]<tmpmat[j][k]
    ...
    for (int i = 0; i < graph->getSize(); i++) {
        fout << "[" << i << "]";
        for (int j = 0; j < graph->getSize() && cout << '\t'; j++) {
            if (tmpmat[i][j] == INF) {
                fout << "\tx ";
            } else {
                fout << "\t" << tmpmat[i][j];
